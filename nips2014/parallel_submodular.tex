...
\begin{figure}[h]
  ...
  \begin{multicols}{2}
    \begin{minipage}{0.45\textwidth}
      \begin{algorithm}[H]
        \DontPrintSemicolon
        \caption{Serial Double Greedy Alg.}
        \label{alg:submax}
        %\Input{}
        $A \leftarrow \emptyset$, $B \leftarrow V$\;
        \For{$i \in \set{1, \ldots n}$ }{
          $\Delta_{A} \leftarrow \max \left( F(A \cup i) - F(A), 0 \right)$\;
          $\Delta_{B} \leftarrow \max \left( F(B \backslash i) - F(B), 0\right)$\;
          Draw $u_i \sim Unif(0,1)$\;
          \lIf{$u_i < \frac{\Delta_A}{\Delta_A + \Delta_B}$ }{
            $A \leftarrow A \cup i$
          }\lElse{
            $B \leftarrow B \backslash i$
          }
        }
      \end{algorithm}
      ...
      \begin{algorithm}[H]
        \DontPrintSemicolon
        \caption{Parallel Double Greedy Alg.}
        \label{alg:par}
        $A \leftarrow \emptyset$, $B \leftarrow V$ \;
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          $i \leftarrow p$ \;
          \While{$i < n$}{
          $(\Gamma_i, \delta_i) \leftarrow T_i(A, B) $ \;
          $(A,B) \leftarrow $ Validate($\delta_i$, $\Gamma_i$) \;
          $i \leftarrow \text{AtomicInc}(i) $
          }
        }
      \end{algorithm}
    ...
    \begin{minipage}{0.45\textwidth}
      ...
      \begin{algorithm}[H]
        \DontPrintSemicolon
        \caption{Coordinate Free Transaction $T_i$}
        \label{alg:cftrans}
        $\Delta_{A} \leftarrow \max \left( F(A \cup i) - F(A), 0 \right)$\;
        $\Delta_{B} \leftarrow \max \left( F(B \backslash i) - F(B), 0\right)$\;
        Draw $u_i \sim Unif(0,1)$\;
      ...
      \begin{algorithm}[H]
        \DontPrintSemicolon
        \caption{Concurrency Control $T_i$}
        \label{alg:cctrans}
        $\Delta_{A} \leftarrow \max \left( F(A \cup i) - F(A), 0 \right)$ \;
        $\Delta_{B} \leftarrow \max \left( F(B \backslash i) - F(B), 0\right)$ \;
        % $\underline{\Delta}_{A} \leftarrow \Delta_A, \bar{\Delta}_{A} \leftarrow \Delta_A,$ \;
        % $\underline{\Delta}_{B} \leftarrow \Delta_B, \bar{\Delta}_{B} \leftarrow \Delta_B,$ \;
       Draw $u_i \sim Unif(0,1)$\;
       $j \leftarrow i+1$ \;
       \If{$u_i < \frac{\Delta_A}{\Delta_A + \Delta_B}$ }{
         \tcp{Construct the tight bound}
         $\bar{A} := A \union \set{i+1, \ldots, j}$ \;
         $\underline{B} := B \backslash \set{i+1, \ldots, j}$ \;
         \While{$u_i < \frac{\Delta_{\bar{A}}}{\Delta_{\bar{A}} + \Delta_{\underline{B}}}$ } {
         $j \leftarrow j + 1$ \;
         }
        ($\Gamma(A',B') = A' \subset \bar{A} \And \underline{B} \subset B'$, +1)
        }\Else{
        \joey{finish other condition}
        (True, -1)
        }
      \end{algorithm}
  ...
  \begin{multicols}{2}
    \begin{minipage}{0.49\textwidth}
      ...
      \begin{algorithm}[H]
        ...
        \WaitUntil $\forall j<i$, res$(j) \neq 0$\;\label{alg:occ:reswait}
        \lIf {res$(i) = 1$}{
      ...
      \begin{algorithm}[H]
        ...
        \ParForAll{$p \in \set{1, \ldots, P}$}{
          \While{$\exists$ element to process}{
            ...
            $\Delta_+^{\min}(e) = F(\tilde{A}) - F(\tilde{A} \backslash e)$\;\label{alg:occ:deltaplusmin}
            $\Delta_+^{\max}(e) = F(\hat{A}   \cup e) - F(\hat{A})$\;\label{alg:occ:deltaplusmax}
            $\Delta_-^{\min}(e) = F(\tilde{B}) - F(\tilde{B} \cup e)$\;\label{alg:occ:deltaminusmin}
...
\begin{lem}\label{lem:hog:set_bound} For any $e\in V$, $\hat{A}_e \subseteq A^{\iota(e)-1}$, and $\hat{B}_e \supseteq B^{\iota(e)-1}$.
\end{lem}
\begin{proof}
